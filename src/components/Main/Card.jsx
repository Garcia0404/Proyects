...
export function Card({nombreProducto, id, precio,image}) {
// cambio de color del corazón
  ...
  function handleAddToCart() {
    addCart({ nombreProducto, id, precio, image });
  }

  return (
    <article className='group/item relative w-48 border border-gray-200 rounded-xl text-center p-3 z-1'>
      <img className='mb-3' src={image} alt={id}/>
      <h3 className='text-sm'>{nombreProducto}</h3>
      <span className='text-blue-800 text-sm'><strong>ID: </strong>{id}</span>
      <button type='button' className='bg-[#131921] text-white block rounded w-11/12 mx-auto p-1 text-nowrap mt-2'>S/{precio}</button>
      ...
      <div className='absolute top-0 start-0 group/edit invisible scale-0 transition-all group-hover/item:visible group-hover/item:scale-100'>
        <div onClick={handleAddToCart}  className='m-2 p-2 rounded-full bg-[#131921] cursor-pointer'>
